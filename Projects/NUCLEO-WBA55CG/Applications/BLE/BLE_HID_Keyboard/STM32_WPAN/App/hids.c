...
typedef struct{
  ...
  uint16_t  FeaturerepCharHdle;                  /**< FEATUREREP Characteristic Handle */
/* USER CODE BEGIN Context */
  ...
  uint16_t  FeatureRepRefDescHdle;         /**< OUTPUTREP REF Descriptor Handle */
...
static const uint16_t SizeOutputrep = 80;
static const uint16_t SizeFeaturerep = 80;
      ...
      switch(p_blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
        {
          ...
          else if(p_attribute_modified->Attr_Handle == (HIDS_Context.HcpCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            ...
            notification.DataTransfered.Length = p_attribute_modified->Attr_Data_Length;
            notification.DataTransfered.p_Payload = &(p_attribute_modified->Attr_Data[0]);
          ...
          else if(p_attribute_modified->Attr_Handle == (HIDS_Context.FeaturerepCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;

            notification.EvtOpcode = HIDS_FEATUREREP_WRITE_EVT;
            ...
            HIDS_Notification(&notification);
          } /* if(p_attribute_modified->Attr_Handle == (HIDS_Context.FeaturerepCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))*/
...
void HIDS_Init(void)
{
  ...
  /* Added 3 Report Reference characteristic descriptors */
  max_attr_record += 3;
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char_desc command   : INPUT REP REF DESC, error code: 0x%2X\n", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char_desc command   : INPUT REP REF DESC on handle 0x%x\n",
                HIDS_Context.InputRepRefDescHdle);
  }
  ...
  ret = aci_gatt_add_char(HIDS_Context.HidsSvcHdle,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_VARIABLE,
                          &(HIDS_Context.OutputrepCharHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("  Fail   : aci_gatt_add_char command   : OUTPUTREP, error code: 0x%2X\n", ret);
  }
  else
  {
    LOG_INFO_APP("  Success: aci_gatt_add_char command   : OUTPUTREP\n");
  }

  /* USER CODE BEGIN SVCCTL_InitService1Char5 */
  /* Place holder for Characteristic Descriptors */
  /**
   * OUTPUTREP REF descriptor
   */
  uuid.Char_UUID_16 = 0x2908;
  buf[0] = 0;
  ...
  ret = aci_gatt_add_char_desc(HIDS_Context.HidsSvcHdle, 
                               ...
                               0x10, 
                               CHAR_VALUE_LEN_CONSTANT,
                               &(HIDS_Context.OutputRepRefDescHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char_desc command   : OUTPUT REP REF DESC, error code: 0x%2X\n", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char_desc command   : OUTPUT REP REF DESC on handle 0x%x\n",
                HIDS_Context.OutputRepRefDescHdle);
  }
  ...
  ret = aci_gatt_add_char(HIDS_Context.HidsSvcHdle,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_VARIABLE,
                          &(HIDS_Context.FeaturerepCharHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("  Fail   : aci_gatt_add_char command   : FEATUREREP, error code: 0x%2X\n", ret);
  }
  else
  {
    LOG_INFO_APP("  Success: aci_gatt_add_char command   : FEATUREREP\n");
  }

  /* USER CODE BEGIN SVCCTL_InitService1Char6 */
  /* Place holder for Characteristic Descriptors */
  /**
   * FEATUREREP REF descriptor
   */
  uuid.Char_UUID_16 = 0x2908;
  buf[0] = 0;
  buf[1] = 3; /* Output Report */
  /* add the valid descriptor */
  ret = aci_gatt_add_char_desc(HIDS_Context.HidsSvcHdle, 
                               HIDS_Context.FeaturerepCharHdle, 
                               UUID_TYPE_16,
                               ...
                               0x10, 
                               CHAR_VALUE_LEN_CONSTANT,
                               &(HIDS_Context.FeatureRepRefDescHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char_desc command   : FEATURE REP REF DESC, error code: 0x%2X\n", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char_desc command   : FEATURE REP REF DESC on handle 0x%x\n",
                HIDS_Context.FeatureRepRefDescHdle);
  }
...
tBleStatus HIDS_UpdateValue(HIDS_CharOpcode_t CharOpcode, HIDS_Data_t *pData)
{
  ...
  switch(CharOpcode)
  {
    ...
    case HIDS_OUTPUTREP:
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        LOG_INFO_APP("  Fail   : aci_gatt_update_char_value OUTPUTREP command, error code: 0x%2X\n", ret);
      }
      else
      {
        LOG_INFO_APP("  Success: aci_gatt_update_char_value OUTPUTREP command\n");
      }
    ...
    case HIDS_FEATUREREP:
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        LOG_INFO_APP("  Fail   : aci_gatt_update_char_value FEATUREREP command, error code: 0x%2X\n", ret);
      }
      else
      {
        LOG_INFO_APP("  Success: aci_gatt_update_char_value FEATUREREP command\n");
      }
